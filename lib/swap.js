...
function detectClaudeProcesses() {
  ...
  try {
    ...
    // anything running out of the LLMSwaper directory, whose path contains "claude".
    const out = execFileSync('pgrep', ['-fl', 'claude'], { encoding: 'utf8', timeout: 5000 });
    const pids = out.split(/\r?\n/)
      .map((line) => line.trim())
      .filter(Boolean)
      .filter((line) => !/llmswaper|server\.js/i.test(line))
 ...
 *
 * organizationName is pinned to "LLMSwaper" rather than left null. Not because there is such
