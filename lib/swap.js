...
function restoreFrom(dir) {
  ...
  if (fs.existsSync(jsonBackup)) {
    fs.copyFileSync(jsonBackup, P.claudeJsonPath());
...
async function swapTo(id, deps) {
  ...
  try {
    ...
    let check = null;
    try {
      check = usage.normalize(await usage.fetchRaw(account.oauth.accessToken), id);
    } catch (err) {
...
module.exports = {
  ...
  writeCredentials, writeClaudeJson, readCurrentIdentity, ensureFreshToken, dryRun, swapTo,
};
