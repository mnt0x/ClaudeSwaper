...
async function swapTo(id, deps, targetId) {
  ...
  try {
    ...
    if (account.profile) {
      mutation = writeClaudeJson(tg.claudeJsonPath, account.profile);
    } else {
      // No identity of our own to write. Leaving the previous one behind would make Claude Code
      // sit there naming the account we just swapped AWAY from — it only reconciles oauthAccount
      // against the token when the token carries user:profile, which this one does not.
      mutation = clearClaudeJsonIdentity(tg.claudeJsonPath);
      if (mutation.clearedIdentity) {
        warnings.push('Token sin perfil: se ha borrado del config la identidad de la cuenta anterior, '
          + 'para que Claude Code no muestre una cuenta que ya no es la activa.');
      }
    }
