...
async function fetchFor(account, { force = false } = {}) {
  ...
  if (Date.now() < cooldownUntil) {
    ...
    if (hit) return staleOf(hit, `rate limit, reintento en ${waitS}s`);
    return { id, ok: false, error: `Rate limit. Reintento en ${waitS}s`, status: 429, needsRelogin: false, retryInS: waitS };
  }
...
async function fetchAll(accounts, opts) {
  const settled = await Promise.allSettled((accounts || []).map((a) => fetchFor(a, opts)));
  const out = {};
  settled.forEach((r, i) => {
...
module.exports = {
  ...
  invalidate, severityFor, cooldownRemainingMs, resetCooldown,
};
