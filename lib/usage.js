...
async function fetchFor(account, { force = false } = {}) {
  ...
  // Checked BEFORE the cache, not after. An account whose token is inference-only can never
  // answer this endpoint, so even a cached reading from an earlier, fuller token would be a
  // number we can no longer stand behind.
  //
  // And it must never reach the network. /api/oauth/usage answers an inference-only token 403
  // — permanently, not transiently — while the whole app gets roughly five calls per five
  // minutes. A 403 is not a 429, so armCooldown never fires and nothing absorbs it: the request
  // would simply repeat on every sweep, spending the shared budget on a guaranteed failure and
  // starving the accounts that CAN answer. This is the single most expensive mistake available
  // in this file, so the guard sits at the top of it.
  // Only an EXPLICIT scope list that lacks user:profile blocks the call. Absent scopes mean an
  // account stored before this mattered, and those came from a full interactive login — assuming
  // the worst there would silently blank dashboards that work fine today. Nothing is guessed in
  // the direction that matters: a pasted token is always stored WITH its scopes, so the only
  // accounts that reach the permissive branch are the legacy ones.
  if (!store.canReadUsage(account)) {
    return {
      id, ok: false, unsupported: true, status: 0, needsRelogin: false,
      error: 'Sin datos de uso: este token solo permite inferencia (user:inference)',
    };
  }
