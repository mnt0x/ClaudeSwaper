...
function pickNext(store, usageMap, targetId, excludeId, threshold) {
  ...
  for (const a of store.list()) {
    ...
    if (!a.oauth || !a.oauth.accessToken) continue;
    if (a.oauth.expiresAt && a.oauth.expiresAt <= Date.now()) continue;
    ...
    if (w != null && w >= threshold) continue;
    candidates.push({ account: a, sort: s == null ? 999 : s });
  }
  candidates.sort((x, y) => x.sort - y.sort);
  return candidates.length ? candidates[0].account : null;
}

/** What the skill/UI shows: on/off, the target, the current account and the next one. */
